...
\subsection*{Lettura delle Figure~\ref{fig:p1_prices} e~\ref{fig:p1_returns}}

I prezzi sono normalizzati a $100$ alla prima data comune: confronto di traiettorie relative senza distorsione da prezzi assoluti. Si osservano cali sincronizzati (2022) e recuperi differenziati: PANW e VRTX oscillano di più; HIG e NDAQ più stabili. La Figura~\ref{fig:p1_returns} sovrappone i log-rendimenti giornalieri di tutti i titoli: si osservano cluster di volatilità comuni (picchi sincronizzati nel 2022 e a fine 2024) e outlier isolati su EW ($-37{,}6\%$, luglio 2024) e PANW ($-33{,}5\%$, febbraio 2024). Queste figure motivano sia l'analisi di volatilità non costante (Punto~2) sia la scelta di confrontare metodi \VaR\ diversi (Punto~3).

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p1_prezzi_normalizzati.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p1_rendimenti_log.pdf}
\caption{Log-rendimenti giornalieri (\%) dei 7 titoli sovrapposti.}
\label{fig:p1_returns}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{p2_vol_statica_vs_roll1y.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p2_vol_pannelli.pdf}
...
\centering
\includegraphics[width=\textwidth]{p2_heatmap_rho.pdf}
\caption{$\hat\rho_{ij}$.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{p2_heatmap_pval.pdf}
...
\textbf{Monte Carlo via Moto Browniano Geometrico (GBM).} La simulazione assume che ciascun prezzo segua il processo $dS/S = \mu\,dt + \sigma\,dW$, da cui il log-rendimento discretizzato su un giorno ($dt = 1/252$):
\[
g_t = \left(\mu_{\text{ann}} - \tfrac{\sigma_{\text{ann}}^2}{2}\right)dt + \sigma_{\text{ann}}\sqrt{dt}\;Z, \qquad Z \sim N(0,1)
\]
I parametri annualizzati sono ricavati dal campione: $\hat\sigma_{\text{ann}} = \hat\sigma_{\text{gg}}\sqrt{252}$ e $\hat\mu_{\text{ann}} = \hat\mu_{\text{gg}} \cdot 252 + \hat\sigma_{\text{ann}}^2/2$, dove il secondo termine è la \textbf{correzione di Itô} che riconduce dalla media dei log-rendimenti al drift del processo di prezzo. Con $N=50\,000$ simulazioni (seed fisso per riproducibilità) il quantile MC converge al parametrico (differenza $<1\%$): la divergenza parametrico/MC segnalerebbe un bug implementativo, non un insight economico. \textbf{Perché GBM e non campionamento diretto da $\mathcal{N}(\hat\mu,\hat\sigma^2)$?} A $H=1$ i risultati sono numericamente identici, ma la formulazione GBM è coerente con il framework di pricing e si estende naturalmente a orizzonti multipli e al caso multivariato (Punto~4.c), dove la decomposizione di Cholesky correla i moti browniani tra titoli.
...
Parametrico $\approx$ MC GBM: validazione implementativa. Storico più basso su EW, VRTX: nel campione le code empiriche sono meno spesse della gaussiana con la stessa $\hat\sigma$ (il \VaR\ storico non ``dà per scontata'' la normalità). PANW massimo ($\approx\$4{,}1$k), coerente con vol Punto~2: il titolo più volatile è anche quello con maggiore perdita potenziale a un giorno. Pannello OOS: eventuali scostamenti dal 5\% invitano a non fidarsi ciecamente del solo parametrico in quei nomi.
...
\begin{tabular}{lrrr}
\toprule
\textbf{Ticker} & \textbf{Param.} & \textbf{Stor.} & \textbf{MC GBM} \\
\midrule
DHR & 2{,}844 & 2{,}679 & 2{,}851 \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p3_var_e_violazioni.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{p4a_portafoglio.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p4b_rendimenti_vol.pdf}
...
\textbf{Monte Carlo GBM multivariato (Cholesky):} si estende la simulazione GBM del Punto~3 al caso multi-asset, correlando i moti browniani dei singoli titoli. La matrice di correlazione $\hat R$ è fattorizzata via Cholesky ($\hat R = LL'$); a ogni simulazione si generano $Z \sim \mathcal{N}(\mathbf{0}, I_7)$, trasformati in shock correlati $\varepsilon = Z \cdot L'$, da cui $g_i = \text{drift}_i + \text{vol}_i \cdot \varepsilon_i$ e $g_p = \sum_i w_i g_i$. $50\,000$ simulazioni, stessa $\hat{\boldsymbol\Sigma}$ del parametrico: verifica numerica (differenza 0,4\%).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p4c_var_ptf.pdf}
...
Media $\hat\rho_{ij}\approx 0{,}23$; tutte le coppie significative al $5\%$. Correlazioni positive $\Rightarrow$ \VaR\ piena (\$1\,467) $>$ indipendenti (\$1\,057): le correlazioni \emph{aumentano} il rischio di circa \$410 ($+39\%$). \textbf{Interpretazione:} in stress i titoli US del campione tendono a scendere insieme; la diversificazione \emph{non} elimina il fattore mercato comune. Il rapporto di diversificazione $1 - \sigma_p / \sum_i w_i\sigma_i = 0{,}424$ indica che la vol del portafoglio è il 42\% inferiore al caso peggiore (tutte le correlazioni unitarie): la composizione settoriale offre un beneficio tangibile, ma il rischio sistematico resta.

Fig.~\ref{fig:p4d}: heatmap; barre parametrico pieno / diagonale / storico.

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p4d_diversificazione.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{p4e_vol_dinamica.pdf}
...
Il lavoro segue la consegna con un filo logico unico: definire bene le grandezze (log-rendimenti, $V_t$ buy-and-hold, $g_p$), misurare dispersione e dipendenza (vol statica/rolling/EWMA/GARCH, Pearson al 5\%), stimare il rischio con tre filosofie (\VaR\ parametrico, storico, MC GBM) e verificarle incrociando numeri e grafici.
...
\textbf{Risultati economici.} Parametrico e MC GBM coincidono (implementazione GBM e ipotesi gaussiana condivise); lo storico è spesso meno severo sul campione (code empiriche). Correlazioni positive alzano il \VaR\ rispetto al caso $\rho_{ij}=0$. La vol del portafoglio non è costante: monitoraggio dinamico necessario.
